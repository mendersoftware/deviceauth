...
func TestProvisionDevice(t *testing.T) {
	...
	ctxWithNoLogs := log.WithContext(context.Background(), l)
	...
	testCases := []testCase{{
		...
		Name: "error, fail to propagate identity to inventory",
		...
		Workflows: func(t *testing.T, self *testCase) *morchestrator.ClientRunner {
			wf := new(morchestrator.ClientRunner)
			wf.On("SubmitProvisionDeviceJob",
				self.CTX,
				mock.AnythingOfType("orchestrator.ProvisionDeviceReq")).
				Return(nil).
				Once().
				On("SubmitUpdateDeviceStatusJob",
					self.CTX,
					mock.AnythingOfType("orchestrator.UpdateDeviceStatusReq")).
				Run(func(args mock.Arguments) {
					req := args.Get(1).(orchestrator.UpdateDeviceStatusReq)
		...
		Error: errors.New("failed to start device inventory update job: internal error"),
	}, {
		Name: "error, failed to submit status to inventory",
		...
		Error: errors.New("failed to submit 'provision_device' workflow"),
	}, {
		...
		Workflows: func(t *testing.T, self *testCase) *morchestrator.ClientRunner {
			return new(morchestrator.ClientRunner)
		},

		Error: ErrDeviceExists,
	}, {
		Name: "error, fail to add device",
		...
		Error: errors.New("failed to add device: internal error"),
	}}
